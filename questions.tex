...
One area where we should dedicate more work on is checking how well different survey strategies enable transient classification. This is similar to the PLAsTiCC challenge where a simulated survey was used to generate light curves and then fed to multiple classifiers. Here, rather than vary the classifiers, we vary the survey realization. 

\begin{itemize}
    \item{Is the survey strategy adequate for classifying transients, or should we place more emphasis on getting more than two observations of a point in a night?}
\end{itemize}