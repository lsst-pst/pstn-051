...
A fraction of twilight time is used for a NEO survey in $r$ band. The NEO survey uses very short (1 second) exposures at high airmass (toward the sun in evening or morning twilight). Note, a NEO survey taking short exposures will drastically increase the data throughput of the system. DM needs to check if this mode of observing would be feasible.  % We also need to check with the camera team that taking short exposures for an extended time will not be a thermal issue. - it should not be, if there are slew breaks between visits as there would be here (it can be an issue if they are continuous without slews)
